...
public class FaceRecognition {
    ...
    public IplImage createIPL(BiometricData d) {
        byte[] b = d.getData();
        return cvDecodeImage(cvMat(1, b.length, CV_8UC1, new BytePointer(b)), CV_LOAD_IMAGE_GRAYSCALE);
    }

    public void learn(List<BiometricData> trainingData) {
        int i;

        trainingFaces = loadFaceImageList(trainingData);
        nTrainFaces = trainingFaces.size();
    ...
    public void recognizeFileList(final String szFileTest) {
        ...
        testFaceImgArr = loadFaceImageList(szFileTest);
     ...
     * @param byteArr List of biometric data objects fetched from database.
     * @return List of IPL images to process with JavaCV
     */
    private List<IplImage> loadFaceImageList(List<BiometricData> byteArr) {
        HashMap<String, Integer> personNumMap = new HashMap<>();

        int nFaces = byteArr.size();
        ...
        for (BiometricData d : byteArr) {
            ...
            /* Store number, name and image */
            faceImageList.add(createIPL(d));
    ...
    private void doPCA() {
        ...
        IplImage[] trainingArray = trainingFaces.toArray(new IplImage[trainingFaces.size()]);
...
//          }
    ...
    private String floatArrayToString(final float[] floatArray) {
        ...
        for (float aFloatArray : floatArray) {
            ...
            stringBuilder.append(aFloatArray);
        }
        stringBuilder.append(']');
